...
- [ ] 记录远程引用并验证仓库外完整历史备份，确认工作区无用户未提交改动。
- [ ] 移除 README 引用、添加精确忽略规则，重写全部可达提交及标签；逐提交验证其他文件与提交元数据不变。
- [ ] 验证本地文档副本不变、历史不再包含目标路径及内容对象、当前文档链接正确。
- [ ] 显式逐引用 lease + atomic 推送 main 和 6 个标签，重新克隆验证远程清理；记录结果与恢复边界。
