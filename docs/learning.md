...
## 2026-09-17 — 压缩镜像应按事件组去重，连线展示应聚合
**Question:** 为什么同一条连线上会出现两个甚至四个压缩标识？
**Key insight:** 一次压缩可能有持久记录和完成通知，时间相差毫秒且夹有状态记录；要求时间字符串相等并只看3条记录会重复计数。保守镜像组结合已知轮次、互补来源、首记录时间窗、活动边界与ID约束，避免把真实多次压缩合并；展示层再独立聚合同边记录，保留逐条明细。
**Details / snippet:** 定向元数据诊断确认19ms、四条状态记录间隔；合成复现及防误合并覆盖同源再现、不同ID/触发方式、未知轮次/时间、用户工具活动、损坏行与容量上限。233项Rust、44项前端与1848/390px Chrome通过。水平边标签高于卡片并用细引线关联，矩形验收确认不会遮挡。所有写入与浏览器验证使用隔离合成数据，真实源文件未改；修复后通过新服务重读历史即可生效。
**Tags:** #compaction #parser #deduplication #canvas #regression #release


## 2026-09-17 — 更新二进制后还要重启旧网页进程
**Question:** 推送3.1.1并更新本地后，如何确保网页实际使用了修复？
...
## 2026-09-23 — 可调面板需要区分首选宽度和实际宽度
**Question:** 如何让左右栏可拖动，同时不破坏画布和窄屏阅读？
