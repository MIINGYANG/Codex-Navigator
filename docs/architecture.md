...
当前版本只交付终端 TUI，没有 HTTP 服务、Web API、网页前端或浏览器启动命令，不能直接通过浏览器查看。本轮仅评估扩展可行性，不创建网络监听。

可复用的核心包括 discovery、domain、parser、watch 和 Prompt 搜索 index；它们不依赖网页。App/ratatui 负责终端键盘和渲染，不宜直接当网页 UI 使用。未来 Web 适配仍需新增只读序列化模型、HTTP 接口、增量事件传输（例如 SSE）和网页界面。

若实现本机 Web，应默认只绑定回环地址，校验 Host/Origin、防止跨站读取，限制会话选择范围及响应体大小；网页必须转义日志内容，不直接渲染未经净化的 HTML。继续只读 Codex 文件，不引入云端、登录、遥测或 AI API。远程/公网访问是另一个安全范围，不能仅把监听地址改成 0.0.0.0。
