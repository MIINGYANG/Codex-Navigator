...
**3.2.1 修复：** 删除前检查会话历史依赖，阻止移走其他分支仍需使用的来源文件。3.2.0 的收藏与跨会话定位功能保持可用；尚未实现 `/btw` 内容收录，侧聊材料仅为设计预览和可行性验证。
...
首次安装和以后更新都用同一条 `cargo install` 命令；`--force` 会替换已有的本项目命令。更新前先用 `Ctrl+C` 停止你启动的网页服务，安装后重新启动并打开新地址；已经运行的进程不会自动升级。确认版本为 **3.2.1**（或更新版本）。
