...
**3.2.0 更新范围：** 本次改进收藏入口与跨会话问题定位，未实现 `/btw` 内容收录。侧聊相关材料仅为设计预览和可行性验证，不包含自动采集、侧聊存档或手动导入功能。
...
首次安装和以后更新都用同一条 `cargo install` 命令；`--force` 会替换已有的本项目命令。更新前先用 `Ctrl+C` 停止你启动的网页服务，安装后重新启动并打开新地址；已经运行的进程不会自动升级。确认版本为 **3.2.0**（或更新版本）。
