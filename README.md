...
首次安装和以后更新都用同一条 `cargo install` 命令；`--force` 会替换已有的本项目命令。更新前先用 `Ctrl+C` 停止你启动的网页服务，安装后重新启动并打开新地址；已经运行的进程不会自动升级。确认版本为 **3.0.0**（或更新版本）。
...
管理操作只支持当前 CODEX_HOME 下 `sessions/` 内的常规文件；显式打开的外部文件仍可浏览。终端版与 `doctor` 保持只读。3.0.0 将 Web 从纯只读扩展为显式管理，安装验收应仅用合成会话测试写操作。
