...
**3.3.0 更新：** 桌面端可拖动左右分隔线调整会话栏和详情栏宽度，并记住偏好。保留3.2.1的删除依赖保护；尚未实现 `/btw` 内容收录，侧聊材料仅为设计预览和可行性验证。
...
- **跨会话找问题**：Question Trail 支持本地问题搜索、前后跳转、原始对话与路径聚焦。侧栏“收藏”可直接查找收藏的问题，无需收藏它所属的会话。
- **跟进进度**：实时更新，翻阅历史时不打断阅读位置。

浏览对话不修改数据，不上传会话，也不需要额外 API Key。收藏只写 Navigator 自有数据；Web 中主动保存名称或确认删除时才修改 Codex 会话管理数据。
...
首次安装和以后更新都用同一条 `cargo install` 命令；`--force` 会替换已有的本项目命令。更新前先用 `Ctrl+C` 停止你启动的网页服务，安装后重新启动并打开新地址；已经运行的进程不会自动升级。确认版本为 **3.3.0**（或更新版本）。
...
- **左右栏宽**：桌面窗口至少1280px时，拖动会话栏右边缘或详情栏左边缘即可调宽；左栏220–420px，右栏300–640px，并为画布保留至少480px。双击分隔线恢复默认；Tab聚焦后用左右方向键微调，按住Shift加速，Home/End调至边界，Enter复位，Escape取消正在进行的拖动。拖动时显示宽度、暂停画布重排；松开后适配新空间。宽度保存在当前浏览器地址下，窗口变窄只限制实际显示，不覆盖首选宽度；窄屏继续使用抽屉与底部详情。
- **排列与密度**：工具栏可切换纵向 / 横向、舒适 / 紧凑。纵向保留双列蛇形，横向根据画布宽度使用每行 2–4 个节点；紧凑主要减少卡片与连线间的空白。真实分支保留已有关系；切换时保留选中的问题。排列偏好保存在当前浏览器地址下。
- **收藏入口**：会话与问题都可独立点击星标。侧栏“收藏”提供“全部 / 问题 / 会话”分类，收藏问题不要求收藏整个会话；问题条目显示来源会话、项目路径和提问时间，可按问题文字、来源名称或路径搜索，点击后直接打开原会话并定位问题。目录覆盖当前数据目录内可发现的历史主会话，索引中、读取上限或原记录不可定位时会明确提示。
- **画布高亮**：收藏继续用金色标记；“星标问题”淡化其他节点并保留上下文连线。侧栏保留“收藏优先”和“有提交”筛选。
...
当前版本尚不自动留存 `/btw` 临时侧聊；它通常没有可供文件监听读取的历史记录。侧聊预览不代表已经接入采集，已确认的接口限制与候选方案见[侧聊接入验证](docs/side-chat-capture.md)。
...
- **删除**：确认框显示名称和项目路径；确认后检查 `sessions/` 与 `archived_sessions/` 内的历史依赖，包括未出现在列表中的子会话。其他会话仍引用该来源文件时拒绝删除，显示依赖数量和会话 ID；检查不完整时也拒绝操作。通过检查后仅将选中的 rollout 文件移到系统回收站，不永久删除、不连带删除。请先结束正在使用或创建相关分支的 Codex 进程；Navigator 无法与外部进程加同一把锁，检查不能排除同时新建分支的竞态。当前支持 Linux 的 `gio trash`，失败时明确报错，不降级为永久删除。
...
如果 Codex 恢复时报 `invalid paginated history lineage ... missing source rollout`，表示它找不到引用的来源文件。旧版本 Navigator 未阻止删除被依赖的来源会话；应核对回收站并恢复原文件，升级本身不会补回历史。见[来源会话恢复排查](docs/session-recovery.md)。不要通过删掉 `history_base` 或改写会话 ID 绕过错误，这会破坏历史关联。
