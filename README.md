...
- **看脉络**：网页版用交互路径串起问题，点开每一步记录，随时回到原文。
...
脉络连线表示**记录先后顺序，不代表因果关系或模型内部思维链**。只展示会话中已有的助手消息、工具活动等记录；分页、搜索间隙和内容省略会明确提示。不调用 AI 推断问题关系，也不恢复未记录或已省略内容。每页最多 100 个问题、每组最多 64 条活动，可继续翻页；进入脉络视图暂停自动跟随，查看历史时新问题不会抢走当前位置。
