...
- **整理会话**：Web 中重命名同步到 Codex；删除经确认后移到系统回收站。
- **找项目**：网页会话列表与当前会话显示完整项目路径，可一键复制后进入对应目录恢复对话。
...
浏览对话不修改数据，不上传会话，也不需要额外 API Key。Web 中只有主动保存名称或确认删除时才执行管理操作。

这不是聊天客户端、AI 总结工具或模型思维链查看器：不生成标题、不推断语义关系，也不能从网页向 Codex 发送提问。标题优先使用 Codex 已保存名称，否则截取已有问题；无法确认分支时显示单主线。
...
首次安装和以后更新都用同一条 `cargo install` 命令；`--force` 会替换已有的本项目命令。更新前先用 `Ctrl+C` 停止你启动的网页服务，安装后重新启动并打开新地址；已经运行的进程不会自动升级。确认版本为 **3.1.1**（或更新版本）。
...
沿用原有问题节点、虚线连线、拖拽缩放和小地图：

- **排列与密度**：工具栏可切换纵向 / 横向、舒适 / 紧凑。纵向保留双列蛇形，横向根据画布宽度使用每行 2–4 个节点；紧凑主要减少卡片与连线间的空白。真实分支保留已有关系；切换时保留选中的问题。排列偏好保存在当前浏览器地址下。
- **会话收藏**：点击会话旁的星标，金色高亮便于回找。侧栏支持“收藏”筛选与“收藏优先”，也可只看“有提交”的会话。问题卡片可单独星标；“星标问题”淡化其他节点，同时保留上下文连线。
- **提交标记**：识别到成功的 Git 提交后，会话侧栏和对应问题显示绿色标记。点击提交徽标或标题下的提交摘要，查看记录中的仓库 / 工作路径、提交时分支、版本标签与提交哈希。缺失字段明确提示未记录；仅凭助手说“已提交”、失败命令或普通日志文字不会标记。
- **上下文压缩**：紫色标记出现在对应问题之后的主线连线上，点击查看时间和触发方式。同一连线的多条事件合并显示为“压缩记录 ×N”，点击打开该段记录列表，再逐条查看详情；列表保留记录数量，不据此推断实际压缩次数。支持自动、手动和“触发方式未记录”三态；旧 Codex 记录常不区分自动 / 手动。工具栏可隐藏标记，“会话事件”仍可查看全部已识别事件，包括末尾或无法可靠定位到连线的压缩。

收藏保存在 Navigator 自己的 `$XDG_DATA_HOME/codex-nav/favorites.json`（默认 `~/.local/share/codex-nav/favorites.json`），不会修改 Codex 会话。使用相同数据目录时，重启服务或更换端口仍保留收藏；恢复到原路径的会话也可找回收藏。问题优先使用原始 turn ID 识别，旧记录回退到序号、时间和问题预览；源文件被改写时，不保证旧问题收藏还能匹配。Unix 支持文件锁与原子保存，其他平台暂不支持保存收藏；存储损坏或不可写时会提示错误，不覆盖原文件。

3.1.1 修复了压缩持久记录与完成通知相差毫秒、夹有状态记录时被重复计数的问题。镜像合并要求同一已知轮次、互补来源、相近时间且中间只有允许的状态记录；有不同事件身份或任务活动边界时保留独立记录。

事件识别只读取会话证据，不执行历史命令、不读取当前 Git 分支来冒充历史信息。仓库字段是提交命令的工作路径，可能为仓库子目录；版本标签只在记录中有成功的显式哈希关联时显示。动态脚本或不支持的工具格式可能无法识别，不能把无标记视为没有提交。每会话最多保留 10,000 个事件，列表摘要受后台索引预算限制；索引未完成时计数未知。

## 在 Web 整理会话

会话列表旁和当前会话标题旁提供改名、删除按钮：

- **重命名**：输入 1–100 个字符并保存，名称同步到 Codex，Navigator 的列表、当前标题与搜索分组同时更新。使用本机 `codex app-server` 的正式 `thread/name/set` 接口，写前核对会话 ID/路径，写后读回；不恢复或运行对话、不修改问题/回复正文。需要 PATH 中可用的 Codex CLI（已验证 0.153.2）。Codex 已打开的旧界面可能需要重新打开列表才能看到名称。
- **删除**：确认框显示名称和项目路径；确认后仅将选中会话的 rollout 文件移到系统回收站，不永久删除，也不连带删除其他会话。请先结束仍在使用该会话的 Codex 进程；Navigator 不能检测所有外部进程的写入状态。当前支持 Linux 的 `gio trash`，不可用或失败时明确报错，不降级为永久删除。
- **恢复**：在系统文件管理器的回收站中将会话恢复到原位置，再重启 Navigator 网页服务。名称记录仍保留在 Codex 中。删除不直接清理 Codex 数据库，Codex 的旧列表可能暂留条目；文件未恢复前无法保证从该条目恢复对话。

重命名和删除只支持当前 CODEX_HOME 下 `sessions/` 内的常规文件；显式打开的外部文件仍可浏览。终端版与 `doctor` 保持只读。3.0.0 将 Web 从纯只读扩展为显式管理，安装验收应仅用合成会话测试写操作。
...
- **没有外传**：Your Codex sessions stay on your computer. Question Trail does not call an AI model. Question Trail does not upload your conversations. 不使用遥测、embedding、运行时 CDN 或远程字体；浏览时不读取认证密钥、不改写 Codex 文件；显式改名交给本机 Codex 子进程保存名称，它可读取自身配置并初始化状态文件。
...
node scripts/management_qa.mjs
node scripts/features_qa.mjs
