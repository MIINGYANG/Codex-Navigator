...
首次安装和以后更新都用同一条 `cargo install` 命令；`--force` 会替换已有的本项目命令。更新前先用 `Ctrl+C` 停止你启动的网页服务，安装后重新启动并打开新地址；已经运行的进程不会自动升级。确认版本为 **3.1.0**（或更新版本）。
...
- **上下文压缩**：紫色标记出现在对应问题之后的主线连线上，点击查看时间和触发方式。支持自动、手动和“触发方式未记录”三态；旧 Codex 记录常不区分自动 / 手动。工具栏可隐藏标记，“会话事件”仍可查看全部已识别事件，包括末尾或无法可靠定位到连线的压缩。
